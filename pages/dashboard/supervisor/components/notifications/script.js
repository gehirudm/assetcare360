class SupervisorNotifications extends HTMLElement {
    constructor() {
        super();
        ...
        this.innerHTML = `
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">Fault ticket updates requiring supervisor attention</p>
                </div>

            </div>

            <div class="supervisor-notifications-toolbar">
                <div class="supervisor-notifications-actions">
                    <button class="btn btn-secondary btn-small" data-action="mark-all-read">Mark All Read</button>
                </div>

                <div class="supervisor-notifications-filters">
                    <div class="supervisor-notifications-filters-head">
                        <h3 class="supervisor-notifications-filters-title"><i class="fas fa-filter"></i> Filter Notifications</h3>
                        <button class="btn btn-secondary btn-small" data-action="clear-filters">Reset Filters</button>
                    </div>

                    <div class="supervisor-notifications-filter-grid">
                        <label class="notifications-filter-field" for="supervisorNotifReadStatusFilter">
                            <span>Read Status</span>
                            <select class="notifications-filter-select" id="supervisorNotifReadStatusFilter" data-filter="readStatus">
                        ...
                        <label class="notifications-filter-field" for="supervisorNotifTypeFilter">
                            <span>Notification Type</span>
                            <select class="notifications-filter-select" id="supervisorNotifTypeFilter" data-filter="type">
                                <option value="all">All</option>
                        ...
                        <label class="notifications-filter-field" for="supervisorNotifSortFilter">
                            <span>Sort By</span>
                            <select class="notifications-filter-select" id="supervisorNotifSortFilter" data-filter="sort">
                                <option value="newest">Newest First</option>
                                <option value="oldest">Oldest First</option>
                            </select>
                        </label>

                        <label class="notifications-filter-field notifications-filter-search" for="supervisorNotifSearchInput">
                            <span>Search</span>
                            <input
                                class="notifications-filter-input"
                                id="supervisorNotifSearchInput"
                                data-filter="search"
                                type="search"
                    ...
                    <p class="supervisor-notifications-filter-summary" id="supervisorNotificationsFilterSummary">No notifications available</p>
                </div>
            </div>

            <div class="supervisor-notifications-list" id="supervisorNotificationsList">
                <div class="notif-empty" id="supervisorNotificationsEmpty" style="display:none;">
                    <i class="fas fa-check-circle"></i>
                    <p id="supervisorNotificationsEmptyText">You're all caught up. No pending notifications.</p>
                </div>
            </div>
        `;
    }
    ...
    syncFilterControls() {
        const readStatusFilter = this.querySelector('#supervisorNotifReadStatusFilter');
        const typeFilter = this.querySelector('#supervisorNotifTypeFilter');
        const sortFilter = this.querySelector('#supervisorNotifSortFilter');
        const searchInput = this.querySelector('#supervisorNotifSearchInput');
    ...
    async loadNotifications() {
        ...
        try {
            ...
            console.error('supervisor-notifications load error:', error);
            this.notifications = [];
            this.updateBadge(0);
            this.setMarkAllDisabled(markAllButton, true);
            this.renderNotifications([], 0, 'Failed to load notifications');

            const list = this.querySelector('#supervisorNotificationsList');
            if (!list) return;
    ...
    renderNotifications(notifications, totalNotifications, summaryOverride = '') {
        const list = this.querySelector('#supervisorNotificationsList');
        const empty = this.querySelector('#supervisorNotificationsEmpty');
        const emptyText = this.querySelector('#supervisorNotificationsEmptyText');
    ...
    updateFilterSummary(visibleCount, totalCount, overrideText = '') {
        const summary = this.querySelector('#supervisorNotificationsFilterSummary');
        if (!summary) return;
    ...
    async markAsRead(notificationId, buttonElement) {
        ...
        try {
            ...
            console.error('supervisor-notifications markAsRead error:', error);
    ...
    async markAllAsRead(buttonElement) {
        ...
        try {
            ...
            console.error('supervisor-notifications markAllAsRead error:', error);
    ...
    dispatchToast(message, type = 'info') {
        this.dispatchEvent(new CustomEvent('supervisor-notifications:toast', {
            bubbles: true,
            detail: {
                message,
                type,
...
if (!customElements.get('supervisor-notifications')) {
    customElements.define('supervisor-notifications', SupervisorNotifications);
}
